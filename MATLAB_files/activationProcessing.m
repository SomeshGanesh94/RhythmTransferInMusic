...
function [new_HD_in, onsets_new_frames, onsets_old_frames] = activationProcessing(HD_in, offset_vector_in, input_to_target, Tempoffset_vector_in, Tempinput_to_target)
...
%             disp(instr);
%         end
%         if length(Tempoffset_vector_in{instr}) >= onset_num
%             tempVar = Tempoffset_vector_in{instr};
%             tempVar(onset_num) = [];
%         end
        if ~ismember(input_to_target{instr}(onset_num), offset_vector_in{instr})
            if input_to_target{instr}(onset_num) ~= offset_vector_in{instr}(onset_num)

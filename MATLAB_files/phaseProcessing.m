...
new_region_idx_cell = regionsAndIdx{2,2};
...
for region = 1 : size(new_rephase,1)
    ...
    new_rephase{region} = pghi(new_region_cell{region}, param.windowSize, param.hopSize, param.windowSize, 'timeinv');
    start_idx = new_region_idx_cell{region}(1);
    end_idx = new_region_idx_cell{region}(2);
    X_complex(:, start_idx:end_idx) = new_rephase{region};
%     X_complex(:, start_idx:end_idx) = 0;
...
% 
%                         end
%                         if frame_no > 80 && frame_no < 82
%                             disp([frame_no,bin_no,spec_max_instr,new_frame_no]);
%                         end
% 
%                     end
% 
